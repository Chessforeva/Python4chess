# polyglot chess opening base functions in python
...
def substr(s,a,b):
 return c0_chess.substr(s,a,b);

def substrall(s,a):
 return c0_chess.substrall(s,a);

def charCodeAt(a,b):
 return c0_chess.charCodeAt(a,b);

def fromCharCode(a):
 return c0_chess.fromCharCode(a);

def IndexOf(s,a):
 return c0_chess.IndexOf(s,a);
...
def readBinFile(filename):
  ...
  fh = fh.close();
  print('Polybase ' + filename + ' in memory');
...
def getKey():
  ...
  pc="pNBRQK";
  i = 0;
  s = c0_chess.c0_position;
  #get key from board
  while i<len(s):
    col = substr(s,i,1);
    piece = substr(s,i+1,1);
    at = substr(s,i+2,2);
    h = charCodeAt(at,0)-97;
    v = charCodeAt(at,1)-49;
    i = i+5;
    x = IndexOf(pc,piece) * 2;
    if col=="w":
      x=x+1;
    k = k ^ Random64[(64*x)+(8*v)+h];

  #castling
  if( not (c0_chess.c0_w00 or c0_chess.c0_wKingmoved ) and not c0_chess.c0_wLRockmoved ):
    k = k ^ Random64[769];
  if( not (c0_chess.c0_w00 or c0_chess.c0_wKingmoved ) and not c0_chess.c0_wRRockmoved ):
    k = k ^ Random64[768];
  if( not (c0_chess.c0_b00 or c0_chess.c0_bKingmoved ) and not c0_chess.c0_bLRockmoved ):
    k = k ^ Random64[771];
  if( not (c0_chess.c0_b00 or c0_chess.c0_bKingmoved ) and not c0_chess.c0_bRRockmoved ):
    k = k ^ Random64[770];
...
def getMove(i):
  global binBase;
  f4 = (binBase[i].move>>6) & 0x77;
  fv = ( f4>>3 ) & 0x7;
  fh = f4 & 0x7;
  to = binBase[i].move & 0x77;
  tv = ( to>>3 ) & 0x7;
  th = to & 0x7;
  p = ( binBase[i].move>>12 ) & 0x7;
  uci = fromCharCode(97+fh)+fromCharCode(49+fv)+fromCharCode(97+th)+fromCharCode(49+tv);
  if(p>1):
    uci = uci + substr("nbrq", p,p);
...
def sample_getAllbyPos(moveslist):

  print("Polybase after: " + moveslist); 
  c0_chess.c0_set_start_position(moveslist);
  s = "";
  ...
  while(j>=0):
    TW = TW + getWeight(j);
    j=j+1;
    if j>=len(binBase) or not (key==binBase[j].key):
      j=-1;
  ...
  while(j>=0):
    move = getMove(j);
    weight = getWeight(j);
    weight = int((10 * weight) / TW)-1;
    if weight<0:
      weight=0;
    print( "Move:" + move + " weight:" + str(weight) );
    s = s + substr(move,0,4) + " (" + fromCharCode(48+weight) + ") ";
    j=j+1;
    if j>=len(binBase) or not (key==binBase[j].key):
      j=-1;
...
#converts move to number
def Move2numb(mv):

  b = 0;
  b = b + (charCodeAt(mv,0)-97) + ((charCodeAt(mv,1)-49)<<3);
  b = b << 6;
  b = b + (charCodeAt(mv,2)-97) + ((charCodeAt(mv,3)-49)<<3);
  return b;

#prepares bytes for writing from given value
def toBinNumb(value,L):
  ...
  while( i<L ):
    ...
    i=i+1;
  if(v>0):
    print("toBinNumb error");
  return a;


# writes a polyglot file
def Write2binfile(filename):

  global binBase;
  print("Writing unsorted polyglot base" + filename);
  ...
  while i<n:
    b1 = bytes(toBinNumb( binBase[i].key, 8));
    fh.write(b1);
    b2 = bytes(toBinNumb( binBase[i].move, 2));
    fh.write(b2);
    b3 = bytes(toBinNumb( binBase[i].weight, 2));
    fh.write(b3);
    b4 = bytes(toBinNumb( binBase[i].learn, 4));
    fh.write(b4);
    i = i + 1;
